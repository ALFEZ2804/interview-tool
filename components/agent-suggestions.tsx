...
const tabs: { id: Tab; label: string; hint: string }[] = [
  {
    id: "killer",
    label: "Killer",
    hint: "Preguntas filtro pensadas para distinguir a un senior de verdad de un senior de currículum. Búscalas cuando dudes entre dos finalistas.",
  },
  {
    id: "technical",
    label: "Técnicas",
    hint: "Profundizan en arquitectura, decisiones y herramientas. Útiles para validar que la experiencia que cuenta el candidato la ha vivido de primera mano.",
  },
  {
    id: "business",
    label: "Business",
    hint: "Miden criterio de negocio, prioridades y comunicación ejecutiva. Útiles aunque el rol sea técnico: revelan cómo razona fuera del editor.",
  },
];
    ...
    <section className="rounded-[var(--radius)] border border-[color:var(--accent-border)] bg-[color:var(--surface)] glow-accent overflow-hidden">
      <header className="px-5 py-4 border-b border-[color:var(--border)]">
        ...
        <h2 className="text-lg font-semibold mt-1.5">
          Preguntas que llevarte a la próxima ronda
        </h2>
        <p className="text-xs text-[color:var(--muted)] mt-1.5 leading-relaxed">
          Tres ángulos distintos calibrados al rol de{" "}
          <span className="text-[color:var(--foreground)] font-medium">
            {roleTitle}
          </span>
          . Cambia de pestaña según qué quieras evaluar mejor la próxima vez.
        </p>
      </header>
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <div className="flex items-center justify-between pt-2 text-[11px] text-[color:var(--muted-2)] border-t border-[color:var(--border)]">
          <span>
            Generadas a partir de la posición y de cómo fue esta entrevista.
          </span>
          <button
            type="button"
